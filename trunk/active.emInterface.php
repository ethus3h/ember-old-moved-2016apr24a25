...
class emInterface
{
    ...
    function cstore($data,$csum) {
    	...
    	$data = dce_convert($data,'binary_raw','3_03a');
    	$csumb = new Csum($data);
    	return store($data,$csumb);
    }
    ...
    function addRecord($flags,$types,$perms) {
		$hash = phash($password);
		$username = amd5($name);
		if(!is_null($this->db->getRowUH('users', 'name', $username))) {
			return false;
		}
		$newRecordId=$this->db->addRow('records', 'latest', '\'0\'');
		$now = microtime();
		$newRevId = $this->db->addRow('recordrevisions', 'record, date, user, owner, flags, types, permissions', '\''.$newRecordId.'\', \''.$this->cstore($now, new Csum($now)).'\', \''.$newRecordId.'\', \''.$this->emUserId.'\', \''.$this->emUserId.'\', \''.$this->cstore($flags, new Csum($flags)).'\', \''.$this->cstore($types, new Csum($types)).'\', \''.$this->cstore($perms, new Csum($perms)).'\'');
		$this->db->setField('records','latest',$newRevId,$newRecordId);
		return $newRecordId;
    }
    ...
    function newLocalisedExec() {
    	if($this->authorized(3)) {
    		$csum = new Csum(fv('intfContent'));
    	    $new = $this->lstore(fv('intfContent'),$csum,fv('locale'));
    		$this->append('Added localised string '.$new['id'].'.');
    	}
    	else {
    		$this->fail('not authorized');
    	}
    ...
    function startForm() {
    	return '<form action="ember.php" method="post" enctype="multipart/form-data"><input type="hidden" name="wint" value="1"><input type="hidden" name="wintNeeded" value="emberWebView"><input type="hidden" name="login" value="1"><input type="hidden" name="emSession" value="'.fv('emSession').'"><input type="hidden" name="locale" value="'.fv('locale').'">';
    }

	function finishForm() {
		return '<center><input type="submit"></center></form>';
	}
...
//  		$this->test($this->ui_breadcrumb_fragment(),'<br><small><a href="ember.php?wintNeeded=emberWebView&wint=1&emAction=1&locale=0">Ember</a> &#x02192; <a href="ember.php?wintNeeded=emberWebView&wint=1&emAction=runTests&locale=0">runTests</a></td></tr></tbody></table><h1>','UI breadcrumb fragment');
    ...
    function ui_logo_fragment() {
		...
		if ($login == 1) {
			$ret=$ret.'&nbsp;&nbsp;(logged in)&nbsp;&nbsp;';
			$ret=$ret.$this->addLink('logOut', '', 'Log out…');
			$ret=$ret.'&#32;';

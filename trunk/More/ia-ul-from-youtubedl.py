...
previousBase = ""
currentBase = ""
access_key = '7d1ZZPu1f4XZg3gF'
secret_key = 'T8CLIjU8zwepZzLV'
myTags = [""]
...
for file in os.listdir(workingDirectory):
	...
	if file.endswith(".info.json") and not file == previousBase:
		...
		if os.path.isfile(jsonFile):
			...
			sanitizeda = commonFile.replace(' ','-')
			sanitized = re.sub(r'[\W_]+', '', sanitizeda)
			...
			if data:
				...
				moretags = "Youtube rip by ET3RN4L;" + sanitizeda.replace('-',';')
				metadata["subject"] = myTags + moretags.split(";")

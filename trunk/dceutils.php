...
================================================================================
                  dceutils. Part of Ember as of 9 June 2014.                    
================================================================================

This is the  main script of dceutils, a library for  conversion to and
...
NOTE:
 ...
 When you're done reading this, edit the file 'config.php' to change this:
...
* Trying for DCE 3.03a support

* Add binary_raw format
...
* Implement keyed storage management in Dc_Data
...
------------------------------------------------+----------------+-------+------
 DCE 3.01a-based formats:                       |                |       |      
                                                |                |       |      
DCE 3.01a**;****                                | 3_01a          | (X)   | (X)     May not work, per comments in code
DCE 3.01a (raw)                                 | dce_3_01a_raw  | (X)   |         May not work, per comments in code
DCE 3.01a (hex-encoded)**;****                  | hex_3_01a      | (X)   | (X)     May not work, per comments in code
DCE 3.01a (raw hex-encoded)                     | hex_3_01a_raw  | (X)   |         May not work, per comments in code
------------------------------------------------+----------------+-------+------
 DCE 3.02a-based formats:                       |                |       |      
                                                |                |       |      
DCE 3.02a**;****                                | 3_02a          | (X)   | (X)     Not really possible, see 3.03a spec
DCE 3.02a (raw)                                 | dce_3_02a_raw  | (X)   |         Not really possible, see 3.03a spec
DCE 3.02a (hex-encoded)**;****                  | hex_3_02a      | (X)   | (X)     Not really possible, see 3.03a spec
DCE 3.02a (raw hex-encoded)                     | hex_3_02a_raw  | (X)   |         Not really possible, see 3.03a spec
------------------------------------------------+----------------+-------+------
 DCE 3.03a-based formats:                       |                |       |      
                                                |                |       |      
DCE 3.03a**;****                                | 3_03a          |  X    |  X      No tests; May not work, per comments in code
DCE 3.03a (raw)                                 | dce_3_03a_raw  |  X    |         No tests; May not work, per comments in code
DCE 3.03a (hex-encoded)**;****                  | hex_3_03a      |  X    |  X      No tests; May not work, per comments in code
DCE 3.03a (raw hex-encoded)                     | hex_3_03a_raw  |  X    |         No tests; May not work, per comments in code
------------------------------------------------+----------------+-------+------
...
UTF-8 (Base64-encoded, DCE 3.01a+ encoding;     | utf8_dc64_bin_ | …     | …
    hex-encoded)                                |  hex           |  X    |  X   
UTF-8 (Base64-encoded, DCE 3.01a+ encoding;     | utf8_dc64_bin_ | …     | …    
    with headers)                               |  enc           |  X    |  X   
UTF-8 (Base64-encoded, DCE 3.01a+ encoding;     | utf8_dc64_bin_ | …     | …    
    with headers; hex-encoded)                  |  enc_hex       |  X    |  X   
------------------------------------------------+----------------+-------+------
 HTML:                                          |                |       |      
                                                |                |       |      
HTML                                            | html           |       |      
...
------------------------------------------------+----------------+-------+------
 Raw:                                           |                |       |      
                                                |                |       |      
Data (binary) (base64 out, no other parsing)    | binary_raw     | (X)   | (X)  
>===============================================^================^=======^=====<
...
Additionally, all of  dceutils 1.43's  functions  are  included  under
their previous names, except  for dce_convert(), which is available as
...
"2.31-"  folder for  the  contents  of  all  the  pre-2.4 versions  of
dceutils. Note that  those  versions  probably do not work  as well as
version 2.4…
...
require ('dceutils_request_handler.php');
require ('dceutils_dc_data_class.php');
log_add('<span style="background-color:magenta;"><strong>Initialized run at ' . $date . '. Log file name: ' . $log_file_name . '<br></strong></span><br>');
//#####################################
...
if ($debug) {
    echo '<br><br>Log file location: <a href="' . $serverprefix . $dceutilsversion . $log_filepath_relative . '">' . $log_filepath_relative . '</a><br><br>';
    $logfile=fopen($log_file_path,"a+");
    fwrite($logfile,gzcompress($html_opening.$log.$html_closing));
    fclose($logfile);
if ($log_show_in_page){

...
================================================================================
                     libdce. Version 2.0, 31 December 2012                      
 and (possibly accidentally) 1 January 2013 (after midnight of 31 December 2012)
================================================================================
...
* Removed Weave dependency.

* Added help function and debug preference.

* Use requires instead of includes where they make sense.

* Improved logging (but file output still doesn't work yet)
...
$tests = true;
//Show error messages? (Can be overridden by $debug.)
$errors = true;
...
================================================+================+=======+======
 CDCE-based formats:                            |                |       |      
                                                |                |       |      
CDCE                                            | cdce           |       |      
CDCE (legacy)***                                | legacy_cdce    |  X    |       Read, lightly tested
CDCE (legacy, strict)*****                      | cdce_lstrict   |  X    | N/A   Read, lightly tested
------------------------------------------------+----------------+-------+------
 Generic DCE formats (automatic version         |                |       |      
     selection):                                |                |       |      
                                                |                |       |      
DCE*                                            | dce            |  X    | X     Read, doesn't work yet
DCE (hex-encoded)                               | hex_dce        |  X    | X    
DCE (raw hex-encoded)                           | hex_dce_raw    |       |      
------------------------------------------------+----------------+-------+------
 DCE 3.0a-based formats:                        |                |       |      
...
------------------------------------------------+----------------+-------+------
 Miscellaneous DCE formats:                     |                |       |      
                                                |                |       |      
Dc ID list (ASCII)***                           | dc             |  X    | X     Both; write tested
------------------------------------------------+----------------+-------+------
 Unicode:                                       |                |       |      
                                                |                |       |      
UTF-8***                                        | utf8           |  X    | X    
...
if ($error_happened) {
    ...
    error_add('<br><font color="red"><strong>' . $error_counter . ' errors were encountered during processing! Review the above list of error messages and/or the log for more information.</strong></font>');
    //}

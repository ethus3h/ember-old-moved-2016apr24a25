<?php
#Futuramerlin Active Scripting Library. Version 0.842, 24 November 2013.
...
function arcmaj3_handler()
{
    ...
    if (Rq('amtask') == 'up') {
        ...
        $ulBarrel = preg_replace('/[\n]+/', "\n", str_replace('\nhttp://http://', 'http://', gzdecode(base64_decode(Rq('uploadedBarrelData')))));
        echo 'ulBarrel:' . "\n\n";
        //print_r($ulBarrel);
        echo "\n\n";
        echo "\n\n";
        $ulFailed   = preg_replace('/[\n]+/', "\n", gzdecode(base64_decode(Rq('failedUrlData'))));
        ...
        foreach ($barrelData as $value) {
            ...
            $potentialProject = '';
            foreach ($pps as $ppid) {
                if (strpos($value, $ppid) !== false) {
                    $testProjects     = True;
                    $potentialProject = $ppid;
                }
            }

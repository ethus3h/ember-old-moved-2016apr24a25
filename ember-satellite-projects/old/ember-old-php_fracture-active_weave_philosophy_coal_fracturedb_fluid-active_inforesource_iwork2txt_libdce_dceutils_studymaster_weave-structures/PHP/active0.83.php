<?php
#Futuramerlin Active Scripting Library. Version 0.83, 23 November 2013.
...
class FractureDB
{
    ...
    function queryInsert($query)
    {
        $this->queryCount++;
        global $db_data;
        //echo "\n\n<br><br><font color=\"red\">EXECUTING QUERY: " . $query . "</font><br><br>\n\n";
        $username = $db_data[$this->name][0];
        $password = $db_data[$this->name][1];
        $db       = new PDO('mysql:host=localhost;dbname=' . "futuqiur_" . $this->name . ';charset=utf8', "futuqiur_" . $username, $password);
        #http://pastebin.com/bbCRpA2m        
    ...
    function getRandomRow($table, $filterField = '', $filterValue = '', $idFieldName = 'id')
    {
        ...
        $query    = 'SELECT * FROM `' . $table . '` WHERE ' . $queryInsert . 'ORDER BY RAND() LIMIT 1;';
        $rowData  = $this->query($query);
        $rowDataP = $rowData[0];
        return $rowDataP;
    }
...
//         $ulFailed   = file_get_contents($_FILES['failedUrlData']['tmp_name']);
        $ulBarrel   = preg_replace('/[\n]+/', "\n",str_replace('\nhttp://http://','http://',hex2bin(Rq('uploadedBarrelData'))));
echo 'ulBarrel:'."\n\n";
        //print_r($ulBarrel);
        echo "\n\n";
        echo "\n\n";
        $ulFailed   = preg_replace('/[\n]+/', "\n",hex2bin(Rq('failedUrlData')));
        ...
        foreach ($failedData as $value) {
            ...
            $currentFailed=$db->getField('am_urls','failedAttempts',$value);
            $currentFailed++;
            $db->setField('am_urls','failedAttempts',$currentFailed,$value);
            $db->setField('am_urls','completed',0,$value);
            $db->setField('am_urls','barrel',0,$value);
        }
        ...
        if (Rq('amtask') == 'down') {
            ...
            $db          = new FractureDB('arcmaj3');
            #Make a new barrel.
            $newBarrelId=$db->addRow('am_barrels','status, who',"'0', '".Rq('userName')."'");
            echo $newBarrelId."\n";
            $urlCounter=0;

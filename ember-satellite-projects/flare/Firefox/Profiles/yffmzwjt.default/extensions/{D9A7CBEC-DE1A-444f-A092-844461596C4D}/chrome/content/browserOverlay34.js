...
var FullZoom = {
  ...
  isMozSyntheticDocument: function(aBrowser) {
    let doc = (aBrowser.getAttribute('remote') == 'true')
              ? aBrowser.contentDocumentAsCPOW
  ...
  isAlreadyApplied: function FullZoom_isAlreadyApplied(aBrowser) {
    ...
    let doc = (browser.getAttribute('remote') == 'true')
              ? browser.contentDocumentAsCPOW
              : browser.contentDocument;
    let index = gBrowser.getBrowserIndexForDocument(doc);
  ...
  _removePref: function FullZoom_removePref(url) {
    let win = (gBrowser.selectedBrowser.getAttribute('remote') == 'true') 
              ? gBrowser.selectedBrowser.contentWindowAsCPOW
              : content;
    if (win instanceof ImageDocument)
      return;
    if (!url)

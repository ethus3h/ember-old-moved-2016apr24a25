...
{
    //This function relies on the lossy UTF8-to-Dc conversion provided by the dceutils 1.43 version included with dceutils 2.0. Once UTF8 to Dc conversion is implemented in dceutils 2.0 this function can be updated.
                ...
                if ((substr($hex, $counter + 0, 8) == '00000040') && (substr($hex, $counter + 24, 8) == '00000040') && ((hex2bin(ltrim(substr($hex, $counter + 8, 8), '0')) . hex2bin(ltrim(substr($hex, $counter + 16, 8), '0'))) > 0) && ((hex2bin(ltrim(substr($hex, $counter + 8, 8), '0')) . hex2bin(ltrim(substr($hex, $counter + 16, 8), '0'))) < 13)) {
                    ...
                    if ($strict) {
                        ...
                        if (strlen($DcMap_Unicode_Lossy[strtoupper(ltrim(substr($hex, $counter, 8), '0')) ]) != 0) {
                            ...
                            error_add('<br><font color="red">Warning: ' . (($counter / 8) + 1) . '. Unicode appended; attempting recovery of corrupted CDCE data: ' . $append . ' → ' . $txt . $append . '<br></font>');
                        } else {
                            $append = '';
                            error_add('<br><font color="red">Warning: ' . (($counter / 8) + 1) . '. Unicode not appended; attempting recovery of corrupted CDCE data' . ' → ' . $txt . $append . '<br></font>');
                        };
                    }
                }
            }
...
function convert_hex_3_03a_to_dc($data)
{
    //DEPENDS: convert_3_03a_to_dc
    return dce_convert(hex2bin($data), '3_03a', 'dc');
}
function convert_hex_3_03a_raw_to_dc($data)
{
    //DEPENDS: convert_3_03a_raw_to_dc
    return dce_convert(hex2bin($data), '3_03a_raw', 'dc');
}
...
function convert_3_03a_raw_to_dc($data)
{
    //DEPENDS: convert_3_03a_to_dc
    if (substr(bin2hex($data), 0, 2) != '80') {
        error_add('<font color="red">Error! This document is not stored using the specified format.</font>');
        ...
        return dce_convert(hex2bin('44434565020103fd') . $data . hex2bin('fd03'), '3_03a', 'dc');
    }
}
...
function convert_utf8_dc64_bin_to_dc($data)
{
    $datahex = bin2hex($data);
    $datalen = strlen($datahex);
    $counter = 0;
    $dclist = '';
    while ($counter < $datalen) {
        global $DcMap_dce3_01a_Core;
        $dclist = $dclist . $DcMap_dce3_01a_Core[strtoupper(substr($datahex, $counter, 2)) ] . ',';
        $counter++;
        $counter++;
    }
    return dce_convert($dclist, 'utf8_dc64', 'dc');
}
function convert_utf8_dc64_bin_hex_to_dc($data)
{
    $datahex = $data;
    $datalen = strlen($datahex);
    $counter = 0;
...
function convert_utf8_dc64_bin_enc_to_dc($data)
{
    return dce_convert(hex2bin(substr(bin2hex($data), 2, -2)), 'utf8_dc64_bin', 'dc');
}
...
function convert_3_03a_to_dc($data)
{
    //This function needs to be improved to fully support DCE 3.01a!
    $hex = bin2hex($data);
    if (substr($hex, 0, 12) !== '444345650201') {
    ...
    if (substr($hex, 12, 2) !== '03') {
        error_add('<font color="red">Error! This document is not stored using the specified version of DCE.</font>');
        return 'This document is not stored using the specified version of DCE.';
    }
    //######################################################
    //This is a DCE 3.03a file.
    global $DcMap_dce3_03a_Core;
    global $DcMap_dce3_03a_Variant_Selectors;
    global $DcMap_dce3_03a_Semantic_Records;
    global $DcMap_dce3_03a_Mathematics;
    global $DcMap_dce3_03a_Whitespace_and_Punctuation;
    global $Dc_to_Base64;
    $counter = 14;
    $txt = '';
    $state = 'Core';
    $varAppend = 'DcMap_dce3_03a_';
    while ($counter < strlen($hex)) {
        $action_last = $action;
        log_add('<br><font color="red">' . ((($counter - 14) / 2) + 1) . ': </font><br>State: ' . $state . '<br>Hex position: ' . substr($hex, $counter, 2) . '<br>Dc ID: ' . $DcMap_dce3_01a_Core[strtoupper(substr($hex, $counter, 2)) ] . '<br>Appending: ');
        $data_array_name = $varAppend . $state;
        ...
        switch ($state) {
        case 'Core':
            if (substr($DcMap_dce3_03a_Core[strtoupper(substr($hex, $counter, 2)) ], 0, 1) == '>') {
                //Switch states
                $state = substr($DcMap_dce3_03a_Core[strtoupper(substr($hex, $counter, 2)) ], 1, strlen($DcMap_dce3_03a_Core[strtoupper(substr($hex, $counter, 2)) ]) - 1);
                $append = '';
                $action = 'State switch out of Core) <br>';
                break;
            } else {
                $append = $DcMap_dce3_03a_Core[strtoupper(substr($hex, $counter, 2)) ] . ',';
                $action = 'Append from Core) <br>';
            }
            break;
        ...
        case 'Variant_Selectors':
            if ((substr($hex, $counter, 2) == 'fd') || (substr($hex, $counter, 2) == 'fe')) {
                ...
                $append = $DcMap_dce3_03a_Variant_Selectors[strtoupper(substr($hex, $counter, 2)) ] . ',';
                $action = 'Append from Variant_Selectors) <br>';
            }
            break;
        ...
        case 'Semantic_Records':
            if ((substr($hex, $counter, 2) == 'fd') || (substr($hex, $counter, 2) == 'fe')) {
                ...
                $append = $DcMap_dce3_03a_Semantic_Records[strtoupper(substr($hex, $counter, 2)) ] . ',';
                $action = 'Append from Semantic_Records) <br>';
            }
            break;
        ...
        case 'Mathematics':
            if ((substr($hex, $counter, 2) == 'fd') || (substr($hex, $counter, 2) == 'fe')) {
                ...
                $append = $DcMap_dce3_03a_Mathematics[strtoupper(substr($hex, $counter, 2)) ] . ',';
                $action = 'Append from Mathematics) . <br>';
            }
            break;
        ...
        case 'Whitespace_and_Punctuation':
            if ((substr($hex, $counter, 2) == 'fd') || (substr($hex, $counter, 2) == 'fe')) {
                ...
                $append = $DcMap_dce3_03a_Whitespace_and_Punctuation[strtoupper(substr($hex, $counter, 2)) ] . ',';
                $action = 'Append from Whitespace_and_Punctuation) <br>';
            }
            break;
        }

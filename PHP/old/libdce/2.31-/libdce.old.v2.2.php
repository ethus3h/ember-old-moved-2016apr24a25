...
================================================================================
                      libdce. Version 2.2, 1 January 2013.                      
Note that the files for  version 2.1 were  misdated as 1 January 2012,
...
* Update an outdated annotation on the legacy_cdce to dc translator

* Improved some error messages, including fixing the way the legacy_cdce to dc
  translator reported its errors (which resulted in error miscounting)

* Fixed a validation bug in the DCE 3.01a translator (it reported non-DCE input
  as DCE input of the wrong version)
...
DCE (hex-encoded)                               | hex_dce        |  X    | X     Read, tested
------------------------------------------------+----------------+-------+------
 DCE 3.0a-based formats:                        |                |       |      
                                                |                |       |      
DCE 3.0a**                                      | 3_0a           | (X)   | X     Read, tested
DCE 3.0a (raw)                                  | dce_3_0a_raw   |       |       Read, tested
DCE 3.0a (hex-encoded)**                        | hex_3_0a       | (X)   | X     Read, tested
...
------------------------------------------------+----------------+-------+------
 DCE 3.01a-based formats:                       |                |       |      
                                                |                |       |      
DCE 3.01a**;****                                | 3_01a          | (X)   | X     Read (partial — no utf8 encapsulation support), tested
DCE 3.01a (raw)                                 | dce_3_01a_raw  |       |       Read, tested
DCE 3.01a (hex-encoded)**;****                  | hex_3_01a      |  X    | X     Read, tested
...
------------------------------------------------+----------------+-------+------
 Unicode:                                       |                |       |      
                                                |                |       |      
UTF-8***                                        | utf8           |  X    | X     Read (partial — no encapsulation support), tested
UTF-32***                                       | utf32          |  X    | X     Read, tested
UTF-8 (Base64-encoded)                          | utf8_base64    |  X    | X     Read, tested
UTF-8 (Base64-encoded, ASCII Dc ID list)****    | utf8_dc64      |  X    | X     Read, very lightly tested
UTF-8 (Base64-encoded, DCE 3.01a+ encoding)**** | utf8_dc64_bin  |       | X    
------------------------------------------------+----------------+-------+------
 HTML:                                          |                |       |      
                                                |                |       |      
